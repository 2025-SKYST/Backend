...
from mysol.app.subscription.dto.responses import SubscriptionDetailResponse, PaginatedSubscriptionResponse
...
from mysol.app.subscription.errors import BlogNotFoundError, SelfSubscriptionError
from mysol.app.user.models import User
from mysol.app.blog.dto.responses import BlogDetailResponse
...
class SubscriptionService:
    ...
    async def get_paginated_subscribed_blogs(self, subscriber: User, page: int, per_page: int)->PaginatedSubscriptionResponse:
        """
        ...
        subscribed_blogs = await self.subscription_store.get_paginated_subscribed_blogs(subscriber_id=subscriber_blog.id, page=page, per_page=per_page)
        blog_responses = [
    ...
    async def get_paginated_subscriber_blogs(self, subscribed: User, page: int, per_page: int)->PaginatedSubscriptionResponse:
        """
        ...
        subscriber_blogs = await self.subscription_store.get_paginated_subscriber_blogs(subscribed_id=subscribed_blog.id, page=page, per_page=per_page)
    ...
    async def get_subscribed_blog_ids(self, subscriber: User) -> List[int]:
        """
        ...
        return await self.subscription_store.get_subscribed_blog_ids(subscriber_blog.id)
    
    async def get_subscriber_blog_ids(self, subscribed: User) -> List[int]:
        """
        나를 구독한 블로그들의 아이디 반환
        """
        subscribed_blog=await self.blog_service.get_blog_by_user(subscribed)
        if not subscribed_blog:
            raise BlogNotFoundError
        return await self.subscription_store.get_subscriber_blog_ids(subscribed_blog.id)
